...
import { DraftInput, NameGate, SceneInlineExtra, YourWords } from "@/components/SpecialScreens";
import { TypewriterText } from "@/components/TypewriterText";
import { scenes } from "@/data/scenes";
import { clearGameState, initialGameState, loadGameState, saveGameState } from "@/lib/storage";
import { personalize, personalizeLines } from "@/lib/text";
...
export default function Home() {
  ...
  function startWithName(playerName: string) {
  ...
  function handleChoice(choice: Choice) {
    ...
    const response = choice.response ? personalize(choice.response, state.playerName) : "";
    const nextState = {
  ...
  return (
    <PhoneFrame noiseLevel={scene.noiseLevel} sceneId={scene.id}>
      <header className={`flex items-center justify-between border-b-4 px-4 py-3 text-[10px] ${scene.id === "TITLE" ? "border-mint/40 bg-ink text-mint" : "border-pixel bg-frost"}`}>
        <span>余白</span>
      ...
      <div className={`flex-1 overflow-y-auto ${scene.id === "TITLE" ? "" : "px-4 py-4"}`}>
        {scene.id === "TITLE" ? (
          <NameGate initialName={state.playerName} onStart={startWithName} onReset={restart} />
        ) : (
          <>
            ...
            <section className="mt-5 space-y-5">
              ...
              {scene.visualType === "your_words" ? (
                ...
                <TypewriterText
                  key={scene.id}
                  lines={personalizeLines(scene.body, state.playerName)}
